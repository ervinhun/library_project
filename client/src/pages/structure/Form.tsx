...
export default function Form({ formType, publicId, open, onClose }: SlideInFormProps) {
    ...
    const [getGenre, setGenre] = useAtom(GenreAtom);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            if (formType === "author") {
                let payload: string | { id: string; name: string }
                let url: string;
                let method: "POST" | "PUT";
                ...
                //if (!response.ok) throw new Error(`Failed to ${editing ? "update" : "add"} author`);

                const data: Author = await response.json();

                if (editing) {
                    setAuthors(prev => prev.map(a => (a.id === publicId ? data : a)));
                } else {
            ...
            if (formType === "genre") {
                ...
                const url = editing ? API_ENDPOINTS.updateGenre : API_ENDPOINTS.addGenre;
                const method = editing ? "PUT" : "POST";

                const response = await fetch(url, {
                    method,
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(editing ? { id: publicId, name: payload } : payload),
                });

                if (!response.ok) throw new Error(`Failed to ${editing ? "update" : "add"} genre`);

                const data: Genre = await response.json();

                if (editing) {
                    setGenre(prev => prev.map(a => (a.id === publicId ? data : a)));

                } else {
                    setGenre(prev => [...prev, data]);
                                ...
                                onChange={(e) => {
                                    ...
                                    if (formType === "author") {
                                        if (selectedAuthors.length > 0) {
                                            ...
                                            setSelectedAuthors([{ id: "", name, createdat: "" }]);
                                        }
                                    } else if (formType === "genre") {
                                        if (selectedGenre) {
                                            setSelectedGenre({ ...selectedGenre, name });
                                        } else {
                                            setSelectedGenre({ id: "", name, createdAt: new Date().toISOString() });
                                        }
                                    }
                                }}
                            />
